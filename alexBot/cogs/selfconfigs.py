from discord.ext import commands
import discord

from ..tools import Cog
from ..tools import get_user_config
from ..tools import BoolConverter
from ..tools import update_user_key


class Configs(Cog):
    """handles guild config settings"""
    @commands.group(name="selfconfig", invoke_without_command=True)
    async def selfconfig(self, ctx: commands.Context):
        """how you configure your guild"""
        await ctx.send(f"see `{self.bot.config.prefix}help` selfconfig for more information")

    @selfconfig.command()
    async def get(self, ctx, key):
        """gets the value of a key"""
        cfg = await get_user_config(self.bot, ctx.author.id)
        try:
            cfg[key]
        except KeyError:
            raise commands.BadArgument(f'the key {key} does not exist')
        await ctx.send(cfg[str(key)])
        pass

    @selfconfig.command()
    async def set(self, ctx: commands.Context, key, value):
        """sets the value of a key"""
        cfg = await get_user_config(self.bot, ctx.author.id)
        try:
            old = cfg[key]
        except KeyError:
            raise commands.BadArgument(f'the key {key} does not exist!')
        if type(old) == bool:
            value = await BoolConverter.convert(self, ctx, value)

        await update_user_key(self.bot, ctx.author.id, key, value)
        try:
            await ctx.message.add_reaction('\U00002705')
        except discord.HTTPException:
            await ctx.send('\U00002705')

    @selfconfig.command()
    async def list(self, ctx):
        """lists the available config keys"""
        cfg = await get_user_config(self.bot, ctx.author.id)
        ret = f'your user config:  ```json\n{cfg}```'
        await ctx.send(ret)


def setup(bot):
    bot.add_cog(Configs(bot))
